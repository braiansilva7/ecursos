...
import { sampleWithRequiredData } from '../tipo.test-samples';
...
describe('Tipo Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.tipos?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to tipoService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getTipoIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getTipoIdentifier).toHaveBeenCalledWith(entity);
