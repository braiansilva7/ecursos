...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('ecursosApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = this.eventManager.subscribe('ecursosApp.httpError', (response: EventWithContent<unknown> | string) => {
  ...
  formatMessage(alert: string): string {
    if (!alert) {
      return 'Mensagem não disponível';
    }
    if (alert === 'error.saramexists') {
      return 'O militar com o SARAM informado já está cadastrado';
    }

    if (alert.startsWith('error.')) {
      alert = alert.substring('error.'.length);
    }
    // Outras verificações ou tratamentos podem ser feitos aqui
    return alert;
  }
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`ecursosApp.${fieldError.objectName as string}.${convertedField}`);
